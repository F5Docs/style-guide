...
Use "if" for a condition of uncertainty — when something may or may not happen. Use "whether" when presenting two or more alternatives. Never use "whether or not" — "whether" alone is sufficient.
...
A simple test: if you can substitute "regardless of whether," use "whether." If you can substitute "in the event that," use "if."

- "Check whether the file is valid" = check regardless of which outcome — use "whether."
- "If the file is invalid, the system will fail" = in the event that this condition is true — use "if."
